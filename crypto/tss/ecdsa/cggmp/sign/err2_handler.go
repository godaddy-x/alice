...
func newErr2Handler(p *round4Handler, reason error) (*err2Handler, error) {
	eh := &err2Handler{
		round4Handler: p,
	...
	if p.err2Msg != nil {
		eh.err2Msgs[p.peerManager.SelfID()] = p.err2Msg
	}
	return eh, nil
}
...
func (p *round4Handler) AbortMessageTypes() []types.MessageType {
...
func (p *round4Handler) OnAbortMessage(logger log.Logger, message types.Message) (types.Handler, error) {
	msg := getMessage(message)
	switch msg.Type {
	case Type_Err1:
		return p.round3Handler.onAbortErr1(logger, message)
	case Type_Err2:
...
func (p *round4Handler) onAbortErr2(logger log.Logger, message types.Message) (types.Handler, error) {
	if err := p.ensureErr2Built(); err != nil {
...
func (p *round4Handler) enterErr2Phase(logger log.Logger, reason error) (types.Handler, error) {
	if err := p.ensureErr2Built(); err != nil {
		logger.Warn("Failed to buildSigmaVerifyFailureMsg", "err", err)
		return nil, err
	}
