...
// matchDecModQWithBetaCorrection verifies DecModQ(C, x) for some count mask
// where x = (share + Σ c_j N_j) mod q. Enumerates c_j∈{0,1} (≤2^8 masks); returns on
// the first VerifyModQ success (early exit — typical case is O(1), worst case 256).
