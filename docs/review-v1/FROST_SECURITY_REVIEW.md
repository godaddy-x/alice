...
> **关联**：[../../crypto/tss/eddsa/frost/README.md](../../crypto/tss/eddsa/frost/README.md)、[CGGMP.md](./CGGMP.md)  
> **修订**：2026-09-12 初版
...
| ID | 级 | 问题 | 位置 | 修复要点 | 优先 |
|----|-----|------|------|----------|------|
| **FR-01** | 中 | Signer 未接入 Echo | `signer/signer.go`、`message.go` | `GetEchoMessage` + `NewEchoMsgMain` | P1 |
...
### 3.1 FR-01：接入 Echo（P1）

`message.go` Round1 echo D/E + `signer.go` 使用 `NewEchoMsgMain`。
